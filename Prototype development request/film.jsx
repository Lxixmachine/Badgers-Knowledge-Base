...
/* ---------- Direct video file with a friendly failure state ---------- */
function FileVideo({ src }) {
  const [err, setErr] = useState(false);
  if (err) return (
    <div className="vthumb vthumb--big lib__empty">
      <div className="lib__emptytxt">
        <span>Film unavailable</span>
        <em>The video file couldn't load. It may not have been uploaded to the site yet — a coach can re-publish or link it from YouTube instead.</em>
      </div>
    </div>
  );
  return <div className="player"><video src={src} controls autoPlay playsInline onError={() => setErr(true)}></video></div>;
}
...
Object.assign(window, {
  ...
  VideoThumb, FilmLibrary, FilmBuilder, FileVideo,
});
