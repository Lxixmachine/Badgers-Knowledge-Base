...
function App() {
  ...
  // keep the URL hash in sync with the open entry (shareable links)
  useEffect(() => {
    try {
      const base = window.location.pathname + window.location.search;
      history.replaceState(null, "", openId ? base + "#e=" + encodeURIComponent(openId) : base);
    } catch { /* file:// or sandboxed — non-fatal */ }
  }, [openId]);
  ...
  async function publishContent() {
    ...
    if (zipper) {
      // One tidy zip: content.js at the root, uploads in films/ and docs/.
    ...
    // Fallback (zip library unavailable): download files one by one.
  ...
  // Lazy-load JSZip from CDN (cached after first use); returns the constructor or null.
  function loadJSZip() {
